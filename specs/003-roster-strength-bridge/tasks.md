...
- [ ] T001 Create the `bridge/` package (`src/vct_moneyball/bridge/__init__.py`) + repo-root
  `artifacts/models/bridge/` dir
- [ ] T002 [P] Register `backfill-sides`, `eval-bridge`, `enc-predict`, `enc-ranking`
...
- [ ] T003 Player-side attribution: map each `player_map_stat` to the match side from the
  parser's `team_abbrev` vs the match's two teams in src/vct_moneyball/bridge/attribution.py
- [ ] T004 [P] `vctm backfill-sides`: offline re-parse to populate `player_map_stat.team_id`
  (idempotent; reports attributed vs. unresolved) in src/vct_moneyball/cli/backfill_sides.py
- [ ] T005 Leakage-free chronological **player Elo** + form + volume replay in
  src/vct_moneyball/bridge/player_rating.py
- [ ] T006 Roster/lineup → team strength as-of (mean/top-k aggregate + confidence) in
...
- [ ] T007 [P] [US1] Unit test: player Elo + roster strength as-of excludes on/after rows
  (no leakage) + determinism in tests/unit/test_player_rating.py
- [ ] T008 [P] [US1] Unit test: roster strength aggregation + low-confidence on sparse rosters
  in tests/unit/test_strength.py
- [ ] T009 [P] [US1] Unit test: roster-strength matchup features (diff encoding) +
  differentiated output for unequal rosters in tests/unit/test_bridge_features.py

### Implementation for User Story 1

- [ ] T010 [US1] Roster-derived matchup features (Elo/form/volume diffs) in
  src/vct_moneyball/bridge/features.py (depends on T006)
- [ ] T011 [US1] Train a calibrated bridge model on club matches (reuse predict/model +
  tracking) and load it in src/vct_moneyball/bridge/model.py (depends on T010)
- [ ] T012 [US1] `vctm enc-predict` (resolve ENC teams, as-of roster strength, calibrated
...
- [ ] T013 [P] [US2] Integration test: `enc-ranking` places all 16 ENC teams with strength +
  confidence and refuses to overwrite an existing artifact in
  tests/integration/test_enc_ranking.py

### Implementation for User Story 2

- [ ] T014 [US2] `vctm enc-ranking` (rank 16 ENC rosters by strength as-of; write dated,
...
- [ ] T015 [P] [US3] Unit test: bridge baselines (pedigree, naive-rating-avg) produce probs on
  the same examples in tests/unit/test_bridge_baselines.py
- [ ] T016 [P] [US3] Integration test: `eval-bridge` writes a report validating against
  contracts/bridge-report.schema.json with model + ≥1 baseline + `leakage_verified` in
  tests/integration/test_eval_bridge.py

### Implementation for User Story 3

- [ ] T017 [P] [US3] Bridge baselines (roster-tier pedigree + naive player-rating average) in
  src/vct_moneyball/bridge/baselines.py
- [ ] T018 [US3] Bridge evaluation (temporal split on club matches, model vs baselines,
...
- [ ] T019 [US1] Single-real-match E2E: attribution + roster strengths + prediction from a
  captured fixture, reproducible (quickstart scenario 1) in
  tests/integration/test_single_match_bridge_e2e.py
- [ ] T020 [P] Document the new `vctm` commands + bridge workflow in services/core/README.md
- [ ] T021 [P] Add Makefile targets `backfill-sides` / `eval-bridge` / `enc-predict` /
  `enc-ranking` in Makefile
- [ ] T022 Run the full quickstart.md validation (scenarios 0–4) end-to-end
- [ ] T023 Confirm `make lint && make fmt && make test` are green
