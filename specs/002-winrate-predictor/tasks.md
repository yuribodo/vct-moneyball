...
- [X] T001 Add `mlflow>=2.14` to the `ml` dependency group and create the `predict/` package
  (`src/vct_moneyball/predict/__init__.py`) + repo-root `artifacts/models/winrate/` in
  services/core/pyproject.toml
- [X] T002 [P] Register `backfill-results`, `train-winrate`, `eval-winrate`, `predict-match`
  subcommands (args per contracts/cli.md, `--json`) in src/vct_moneyball/cli/main.py
- [X] T003 [P] Ignore `mlruns/` (MLflow local store) in repo .gitignore
...
- [X] T004 Add match identity/outcome columns (`team_a_id`, `team_b_id`, `winner_team_id`,
  `score_a`, `score_b` + indexes) to the `Match` model in src/vct_moneyball/store/models.py
- [X] T005 Generate the Alembic migration for the new `match` columns in
  services/core/alembic/versions/
- [X] T006 [US-prep] Extend the VLR match parser to read both teams (`vlr_team_id`), series
  score, and winner from the match header in src/vct_moneyball/collect/parse.py
- [X] T007 Repository upserts for the match outcome + a `team` row per side (clubs,
  `is_enc_2026=false`, with provenance) in src/vct_moneyball/store/repositories.py
- [X] T008 [P] `vctm backfill-results`: offline re-parse of cached HTML to populate match
  outcomes (idempotent; reports labeled vs. unlabeled) in src/vct_moneyball/cli/backfill_results.py
- [X] T009 [P] MLflow run wrapper + dataset/feature/config fingerprint in
...
- [X] T010 [P] [US1] Unit test: outcome label (team_a win) from stored result in
  tests/unit/test_labels.py
- [X] T011 [P] [US1] Unit test: as-of feature builder excludes any row dated on/after the
  match (no leakage) + determinism in tests/unit/test_features.py
- [X] T012 [P] [US1] Unit test: train→calibrate→predict_proba returns probabilities summing
  to 1.0; low-history flag set in tests/unit/test_model.py

### Implementation for User Story 1

- [X] T013 [P] [US1] Match outcome labels in src/vct_moneyball/predict/labels.py (depends on T007)
- [X] T014 [US1] Leakage-free as-of opponent-difference feature builder in
  src/vct_moneyball/predict/features.py (reuses score/player + config)
- [X] T015 [US1] Calibrated model (regularized logreg + `CalibratedClassifierCV`),
  `predict_proba`, low-confidence flagging in src/vct_moneyball/predict/model.py (depends on T014)
- [X] T016 [US1] `vctm train-winrate` (train before cutoff, log MLflow run + artifact) in
  src/vct_moneyball/cli/train_winrate.py (depends on T015, T009)
- [X] T017 [US1] `vctm predict-match` (resolve teams, as-of features, probabilities + winner +
...
- [X] T018 [P] [US2] Unit test: temporal split has zero overlap, no match straddles cutoff,
  `leakage_verified` true in tests/unit/test_dataset.py
- [X] T019 [P] [US2] Unit test: metrics (log-loss, accuracy, Brier, calibration) + baselines
  in tests/unit/test_eval_metrics.py
- [X] T020 [P] [US2] Integration test: `eval-winrate` writes a report validating against
  contracts/eval-report.schema.json with model + ≥1 baseline in
  tests/integration/test_eval_winrate.py

### Implementation for User Story 2

- [X] T021 [US2] Temporal split + atomic-match + leakage verification in
  src/vct_moneyball/predict/dataset.py (depends on T014)
- [X] T022 [P] [US2] Explicit baselines (roster-tier pedigree, power-rank favorite,
  win-rate/Elo) in src/vct_moneyball/predict/baselines.py
- [X] T023 [P] [US2] Evaluation metrics (log-loss, accuracy, Brier, calibration error) in
  src/vct_moneyball/predict/evaluate.py
- [X] T024 [US2] `vctm eval-winrate` (forward-chaining split, model vs. baselines, write
...
- [X] T025 [P] [US3] Integration test: a completed run records data window, feature/config
...
- [X] T026 [US3] Ensure train/eval log params + data window + dataset/feature fingerprint +
...
- [X] T027 [US1] Single-real-match E2E: label + as-of features + prediction from a captured
  fixture, reproducible (quickstart scenario 1) in tests/integration/test_single_match_pred_e2e.py
- [X] T028 [P] Document the new `vctm` commands + model workflow in services/core/README.md
- [X] T029 [P] Add Makefile targets `backfill-results` / `train-winrate` / `eval-winrate` /
  `predict-match` in Makefile
- [X] T030 Run the full quickstart.md validation (scenarios 0–4) end-to-end
- [X] T031 Confirm `make lint && make fmt && make test` are green
